...
function tratar_eventos() {
    ...
    document.getElementById("form-contato").onsubmit = function () {
        ...
        img_remove.setAtribute('src', 'img/cross.png');
        ...
        img_avatar.setAtribute('src', 'img/' + this.avatar_selecionado.value);
        ...
        document.getElementById("tabela-contatos").getElementsByTagName("tbody")[0].appendChild(tr);

        document.getElementById("formulario-cadastro-contato").style.display = "none";
        document.getElementByNameById("mostra-form-cadastro").style.display = "block";
...
function ativar_botoes_exclusao() {
    ...
    for (var i = 0; i < imagens_tabela.length; i++) {
        ...
        if (imagens_tabela[i].className == "botao-remover-contato") {

            imagens_tabela[i].onclick = function () {

                if (confirm("Você realmente deseja excluir este contato?")) 
                {
